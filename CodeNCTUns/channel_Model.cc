...
channelModel::Pr(struct cmInfo *cminfo){
	...
	switch(PropModel){
		case FREE_SPACE:
			Pr = 10 * log10(Friis(Ptmw, Gt, Gr, Lambda, L, dist,PLExp_)); // dbm
			if(fadingOpt == 1){
...
//
//<- double
double channelModel::RiceanFading(double Pr, int txNid, int rxNid)
{
...
channelModel::TRG_FS(double Pt, double Gt, double Gr, double ht, double hr, double L, double d)
{
	 ...
	 */
  	...
  	if (d <= crossover_dist) {
		Pr0 = Friis(Pt, Gt, Gr, Lambda,  L, d,2); // mWatt
 		Pr = 10 * log10(Pr0); 
 		return(Pr); // dbm
	}
	else {
		...
		double pathlossExp_, double std_db_, double dist0_)
{
	...
	double Pr0 = Friis(Pt, Gt, Gr, Lambda, L, dist0_,2); // mWatt
	...
	// the power loss is relative to that at reference distance dist0_
        ...
        double powerLoss_db = avg_db + 1;
        
        //double powerLoss_db =46.6777;
...
double
channelModel::Friis(double Pt, double Gt, double Gr, double lambda, double L, double d,double exp) {
	/* 
	...
	// The unit of the returned value is depends on the unit of Pt (Watt or milli-Watt) 
  	return ((Pt * Gt * Gr * (M * M)) /(pow(d,exp) * L)); 
}
...
double channelModel::Calculate_Ptx(struct cmInfo *cminfo, double CSThresh, double dist){
	if(!cminfo){
		fprintf(stderr, "[%s] Error: No cminfo data\n", __func__);
		return 0;
	}
